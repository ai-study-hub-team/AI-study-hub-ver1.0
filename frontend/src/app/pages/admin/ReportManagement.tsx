  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900">Report Management</h1>
          <p className="text-slate-500">Review and resolve content reports from users</p>
        </div>
        {pendingCount > 0 && (
              ...
              {reportCategories.slice(0, 4).map((cat) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all ${
                    categoryFilter === cat ? "bg-blue-600 text-white" : "bg-slate-50 text-slate-600 hover:bg-slate-100"
                  }`}
                >
                  {cat}
                </button>
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-slate-900 text-sm truncate">{report.document}</span>
                  </div>
          ...
          {selectedReport ? (
            ...
            >
              <div className="flex items-start justify-between mb-6">
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{selectedReport.id}</span>
                  <h2 className="text-xl font-extrabold text-slate-900 mt-1">{selectedReport.document}</h2>
                </div>
              ...
              <div className="space-y-4 mb-6">
                <div className="flex gap-3 p-3 bg-slate-50 rounded-2xl">
                  <div>
                    ...
                    <p className="font-semibold text-slate-800 text-sm">{selectedReport.category}</p>
                  </div>
                </div>
                <div className="flex gap-3 p-3 bg-slate-50 rounded-2xl">
                  <div>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-0.5">Reported By</p>
                    <p className="font-semibold text-slate-800 text-sm">{selectedReport.reporter} · {selectedReport.date}</p>
                  </div>
                </div>
                <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl">
                  <p className="text-xs text-amber-600 font-bold uppercase tracking-wider mb-2">Description</p>
                  <p className="text-sm text-amber-800 leading-relaxed">{selectedReport.description}</p>
                </div>
              </div>
              ...
              {selectedReport.status === "pending" ? (
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className="w-full py-3 border border-slate-200 text-slate-600 font-bold rounded-2xl hover:bg-slate-50 transition-colors flex items-center justify-center gap-2"
                  >
                ...
                <div className={`p-4 rounded-2xl flex items-center gap-3 ${
                  selectedReport.status === "resolved" ? "bg-emerald-50 border border-emerald-100" : "bg-slate-50 border border-slate-100"
                }`}>
                  {selectedReport.status === "resolved"
                    ? <CheckCircle2 className="w-5 h-5 text-emerald-500" />
                    : <XCircle className="w-5 h-5 text-slate-400" />}
                  <p className={`font-bold text-sm capitalize ${selectedReport.status === "resolved" ? "text-emerald-700" : "text-slate-600"}`}>
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center p-12 text-center"
            >
              <Flag className="w-12 h-12 text-slate-200 mb-4" />
              <p className="font-bold text-slate-400">Select a report to review</p>
              <p className="text-sm text-slate-300 mt-1">Click any report from the list to see details</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>

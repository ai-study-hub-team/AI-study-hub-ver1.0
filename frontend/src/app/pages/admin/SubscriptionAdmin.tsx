    ...
    <div className="space-y-8 bg-slate-50 dark:bg-slate-950">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Revenue tracking, plan distribution, and subscription analytics</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        {kpis.map((kpi, i) => (
          ...
          >
            <div className={`w-10 h-10 rounded-xl mb-3 flex items-center justify-center bg-${kpi.color}-50 dark:bg-slate-800`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm p-6 dark:[&_.recharts-cartesian-axis-tick_text]:fill-slate-400 dark:[&_.recharts-cartesian-grid_line]:stroke-slate-800 dark:[&_.recharts-default-tooltip]:!bg-slate-900 dark:[&_.recharts-default-tooltip]:!border-slate-700 dark:[&_.recharts-tooltip-label]:!text-white dark:[&_.recharts-tooltip-item]:!text-slate-300">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Monthly Revenue</span>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm p-6 dark:[&_.recharts-default-tooltip]:!bg-slate-900 dark:[&_.recharts-default-tooltip]:!border-slate-700 dark:[&_.recharts-tooltip-label]:!text-white dark:[&_.recharts-tooltip-item]:!text-slate-300">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-5">Plan Distribution</h2>
            ...
            {planDistribution.map((plan) => {
              ...
              return (
                <div key={plan.name} className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold text-slate-700 dark:text-slate-300 w-16 text-right">{plan.value.toLocaleString()}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Recent Upgrades</h2>
          ...
          <table className="w-full border-separate border-spacing-y-2">
            ...
            <tbody>
              {recentUpgrades.map((upg, i) => (
                <tr key={i} className="group bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  <td className="px-4 py-3 rounded-l-2xl">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-slate-900 dark:text-white text-sm">{upg.user}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-bold rounded-lg">{upg.from}</span>
                      <ArrowUpRight className="w-3 h-3 text-emerald-500" />
                      <span className={`px-2 py-0.5 text-xs font-bold rounded-lg ${upg.to === "Pro" ? "bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-300" : "bg-purple-50 dark:bg-purple-500/10 text-purple-600 dark:text-purple-300"}`}>{upg.to}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-sm text-slate-500 dark:text-slate-400">{upg.date}</td>

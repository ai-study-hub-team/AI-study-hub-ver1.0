...
function DocumentRow({
  ...
  return (
    <div className="grid gap-3 border-t border-slate-100 bg-white p-4 transition-colors hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800/70 md:grid-cols-[minmax(0,2fr)_minmax(120px,1fr)_minmax(120px,1fr)_minmax(120px,1fr)_auto] md:items-center">
      ...
      <div>
        <p className="text-[11px] font-bold uppercase text-slate-400 dark:text-slate-500 md:hidden">
          Category
        </p>
      ...
      <div className="flex items-center justify-end gap-1">
        ...
        <button
          onClick={() => toast.info("Document actions coming soon.")}
  ...
  const filteredDocuments = useMemo(() => {
    ...
    if (normalizedQuery) {
      result = result.filter((document) =>
        [document.name, document.folder, document.type, document.aiStatus].some(
    ...
    if (statusFilter !== "ALL") {
      result = result.filter((document) => document.aiStatus === statusFilter);
    }
    ...
    return result;
  }, [documents, searchQuery, statusFilter, sortOrder]);
          ...
          <div className="relative">
            ...
            {showFilterMenu && (
              <div className="absolute right-0 z-50 mt-2 w-44 rounded-xl border border-slate-200 bg-white p-2 shadow-lg dark:border-slate-700 dark:bg-slate-900">
                {["ALL", "READY", "PROCESSING", "FAILED", "PENDING"].map(
                  (status) => (
                    <button
                      key={status}
                      onClick={() => {
                        setStatusFilter(status as AiStatus | "ALL");
      ...
      <section>
        ...
        {view === "grid" ? (
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="hidden bg-slate-50 px-4 py-3 text-[11px] font-extrabold uppercase text-slate-400 dark:bg-slate-800/60 dark:text-slate-500 md:grid md:grid-cols-[minmax(0,2fr)_minmax(120px,1fr)_minmax(120px,1fr)_minmax(120px,1fr)_auto]">
              <span>Name</span>
              <span>Category</span>
              <span>Uploaded</span>
              <span>Status</span>

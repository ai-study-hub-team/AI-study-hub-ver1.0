...
export function TrashPage() {
  ...
  const handleRestore = async (document: TrashDocument) => {
    if (!window.confirm(`Restore “${document.name}” to your library?`)) return;
  ...
  const handleDeleteForever = async (document: TrashDocument) => {
    if (!window.confirm(`Permanently delete “${document.name}”? This action cannot be undone.`)) return;
    setPendingId(document.id);
    try {

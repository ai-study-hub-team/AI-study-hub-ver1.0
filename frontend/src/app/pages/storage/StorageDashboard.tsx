...
const FileTypeIcon = ({ type }: { type: string }) => {
  const map: Record<string, { icon: React.ElementType; color: string }> = {
    ...
    video: { icon: Film, color: "text-amber-500 bg-amber-50" },
    archive: { icon: Archive, color: "text-slate-500 bg-slate-100" },
  };
  const { icon: Icon, color } = map[type] || { icon: FileText, color: "text-slate-500 bg-slate-100" };
  return (
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900">Cloud Storage</h1>
          <p className="text-slate-500">Manage your files, storage quota, and uploads</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Quota Gauge */}
        <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 flex flex-col items-center">
          <h2 className="text-lg font-bold text-slate-900 mb-4 self-start">Storage Quota</h2>
          ...
          <div className="mt-4 text-center">
            <p className="text-2xl font-extrabold text-slate-900">{USED_GB} GB</p>
            <p className="text-sm text-slate-500">of {TOTAL_GB} GB used</p>
          </div>
          <div className="w-full mt-6 bg-slate-50 rounded-xl p-3 border border-slate-100">
        ...
        <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6">
          <h2 className="text-lg font-bold text-slate-900 mb-5">By File Type</h2>
              ...
              <div key={cat.name} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-slate-400 ml-2 shrink-0">{cat.size}</span>
                  </div>
          ...
          <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between">
            <span className="text-sm text-slate-500">198 total files</span>
        ...
        <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-bold text-slate-900">Upload Queue</h2>
            <span className="px-2.5 py-0.5 bg-blue-50 text-blue-600 text-xs font-bold rounded-lg">{activeQueue.filter(q => q.status !== "done").length} pending</span>
          </div>
          <div className="space-y-3">
            <AnimatePresence>
            ...
            <tbody>
              {recentFiles.map((file) => (
                <tr key={file.id} className="group hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-slate-500">{file.modified}</td>

...
public class DocumentAccessService {
    ...
    public Document getAccessibleDocument(Long documentId) {
        return getAccessibleDocument(currentUserService.getCurrentUser(), documentId);
    }
    ...
    public boolean canAccessDocument(User user, Document document) {
        ...
        if (document.getUser() != null && user.getId().equals(document.getUser().getId())) {
            return true;
        }
        ...
        return documentShareRepository.existsByDocumentIdAndSharedWithId(document.getId(), user.getId());
    }
